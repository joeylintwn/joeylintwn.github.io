  ...
  ArcGISTiledMapServiceLayer) {
    ...
    */
     ...
     postCreate: function () {
       ...
       if (this.legend) {
         ...
         if (this.serviceLayer.name != '禁挖區' && this.serviceLayer.Show == 1) {
     ...
     _createServiceLayerNode: function (serviceLayer) {
       ...
       var temp = serviceLayer.name.split('_');
 
       if (temp[1] != null) {
         this.labelNode.innerHTML = temp[1];
       }
       else {
       ...
       if (serviceLayer._subLayerInfos) {// group layer
         ...
         var subLayerInfos = dojo.filter(serviceLayer._subLayerInfos, function (item) {
           return item.Show == 1;
         });
         this._createChildrenNodes(subLayerInfos, 'serviceLayer');
     ...
     _onClick: function (evt) {
       ...
       if (t == this.checkNode || dijit.getEnclosingWidget(t) == this.checkNode) {
         ...
         if (this.serviceLayer) {
           ...
           if (this.rootLayer.custmer != 1) {
               if (this.rootLayer.visible) {
                   if (!this.rootLayer instanceof esri.layers.WMSLayer) {
                       if (this.rootLayer.layerInfos != null) {
                           for (i = 0; i < this.rootLayer.layerInfos.length; i++) {
                               this.rootLayer.layerInfos[i].visible = true;
                               this.rootLayer.setVisibleLayers(this._getVisibleLayers(), true);
                           }
                       }
                   }
                   ...
                   if (this.rootLayer.layerInfos != null) {
                       for (i = 0; i < this.rootLayer.layerInfos.length; i++) {
                           this.rootLayer.layerInfos[i].visible = false;
                       }
                       if (!this.rootLayer instanceof esri.layers.WMSLayer) {
                           this.rootLayer.setVisibleLayers(this._getVisibleLayers(), true);
                       }
               ...
               if (this.rootLayer.visible) {
                   for (i = 0; i < this.rootLayer.layerInfos.length; i++) {
                       this.rootLayer.layerInfos[i].visible = true;
                       this.rootLayer.setVisibleLayers(this._getcustomerVisibleLayers(), true);
                   }
               }
               else {
     ...
     _getcustomerVisibleLayers: function () {
         var vis = [];
         array.forEach(this.rootLayer.layerInfos, function (layerInfo) {
             if (layerInfo.subLayerIds) {
                 ...
                 return;
             } else if (layerInfo.visible && layerInfo.Show == 1) {
   ...
   var _RootLayerTOC = declare([_Widget], {
     ...
     _getLegendInfo: function () {
       ...
       var url = '';
       if (this.rootLayer.version == null || this.rootLayer.version >= 10.01) {
