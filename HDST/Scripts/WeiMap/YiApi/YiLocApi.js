var _YISEARCHDATAURL = "https://map.e-land.gov.tw/yigis/ws_dataC.ashx";
    ...
    esri.request({
        ...
        load: function (json) {
            if (json[0].ABOUT == "") {
                ...
                //yilandmap.centerAndZoom(pt, 0);
                yilandmap.setScale(12000);
                yilandmap.centerAt(pt);
            }
            else {
                alert(json[0].ABOUT);
            }
    ...
    if (featureSet.features.length == 0) {
        ...
        if (featureSet.geometryType == "esriGeometryPoint") {
            ...
            var symbol = new esri.symbol.SimpleFillSymbol(esri.symbol.SimpleFillSymbol.STYLE_SOLID,
                ...
                new dojo.Color([255, 0, 0, 1]), 2), new dojo.Color([255, 0, 0, 0.3]));
            for (var j = 0, jl = featureSet.features.length; j < jl; j++) {
                //var graphic = featureSet.features[j];
                var graphic = new esri.Graphic(featureSet.features[j]);
                graphic.setSymbol(symbol);
