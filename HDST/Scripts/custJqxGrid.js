...
$(window).ready(function () {
    /*
     by jay 由於有部分頁面會有路徑問題
     所以有些套件不需要從這邊載入
     */
    var isLoad = isLoad1 = isLoad2 = isLoad3 = true;

    var Allstyle = document.getElementsByTagName("link");

    for (var i = 0; i < Allstyle.length; i++) {
        var href = Allstyle[i].href;
        if (href !== undefined) {
            if (href.indexOf("jqx.base.css") >= 0) {
                isLoad = false;
            }

            if (href.indexOf("jqx.custom.css") >= 0) {
                isLoad1 = false;
            }
        }
    }
    ...
    for (var i = 0; i < Allscript.length; i++) {
        ...
        if (src !== undefined) {
            if (src.indexOf("jqxcore.js") >= 0) {
                isLoad2 = false;
            }

            if (src.indexOf("jqx-all2.js") >= 0) {
                isLoad3 = false;
            }

            if (src.indexOf("jqxcheckbox.js") >= 0) {
                isLoadJqxGrid = true;
            }
        } 
    }
